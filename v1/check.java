...
public class check {
    ...
    public String getNeverSoldProductCount() {
        StringBuilder sb = new StringBuilder();
        try {
            NeverSoldProductCount = con.prepareStatement("select count(m1.model_id) as number from (select model_id, " +
                    "sum(stock_quantity) as stoNum from stock group by model_id)m1 join stock on stoNum = tot_quantity ");
